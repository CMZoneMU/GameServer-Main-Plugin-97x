...
bool cDuelEvent::Load()
{
	...
	Token.ParseFile(std::string("./Data/DuelEvent.txt"), Group);
		...
		if (!_stricmp(Text, this->_Syntax[0]))
		{
			if (this->_State == Register && lpObj->Live == TRUE)
			{
				if (this->_Type != 4 && lpObj->Class != this->_Type)
				{
					...
					{
					case 0:
						Func.MsgOutput(lpObj->m_Index, 1, "Mata-Mata exclusivo para Dark Wizards e Soul Masters");
						break;
					case 1:
						Func.MsgOutput(lpObj->m_Index, 1, "Mata-Mata exclusivo para Dark Knights e Blade Knights");
						break;
					case 2:
						Func.MsgOutput(lpObj->m_Index, 1, "Mata-Mata exclusivo para Elfs e Muse Elfs");
						break;
					case 3:
						Func.MsgOutput(lpObj->m_Index, 1, "Mata-Mata exclusivo para Magic Gladiators");
						break;
					}
...
void cDuelEvent::Run()
{
	...
	case Register:
	{
		if (this->_Count == 0)
		{
			...
			else
			{
				...
				sprintf_s(DuelEvent._Buffer[0], "Inscrições para o evento encerradas.");
				sprintf_s(DuelEvent._Buffer[1], "PRIMEIRA RODADA!");

				for (int i = OBJECT_MIN; i < OBJECT_MAX; i++)
				{
					if (gObj[i].Connected == 3)
					{
						GCServerMsgStringSend(DuelEvent._Buffer[0], i, 0);
						GCServerMsgStringSend(DuelEvent._Buffer[1], i, 0);
					}
				}
		...
		else
		{
			if ((this->_Count % 60) == 0)
			{
				sprintf_s(DuelEvent._Buffer[0], "Mata-Mata começará em %d Minuto(s)", (this->_Count / 60));
				sprintf_s(DuelEvent._Buffer[1], "Para participar, utilize o comando: %s", this->_Syntax[0]);
	...
	case Select:
	{
		if (this->_Count == 0)
		{
			...
			SelectWO:
				...
				sprintf_s(DuelEvent._Buffer[0], "[    DuelEvent    ]");

				sprintf_s(DuelEvent._Buffer[1], "%s venceu de W.O.", this->_Selected[0].lpObj->Name);
			...
			default:
			{
				...
				else
				{
					...
					sprintf_s(DuelEvent._Buffer[1], "%s  vs  %s", this->_Selected[0].lpObj->Name, this->_Selected[1].lpObj->Name);
	...
	case Died:
	{
		if (this->_Count == 0)
		{
			...
			else
			{
				...
				sprintf_s(DuelEvent._Buffer[0], "[    PLACAR    ]");

				sprintf_s(DuelEvent._Buffer[1], "%s  vs  %s", this->_Selected[0].lpObj->Name, this->_Selected[1].lpObj->Name);

				sprintf_s(DuelEvent._Buffer[2], "%d   x   %d", this->_Selected[0].Score, this->_Selected[1].Score);
				...
				for (int i = OBJECT_MIN; i < OBJECT_MAX; i++)
				{
					if (gObj[i].Connected == 3)
					{
						GCServerMsgStringSend(DuelEvent._Buffer[0], i, 0);
						GCServerMsgStringSend(DuelEvent._Buffer[1], i, 0);
						GCServerMsgStringSend(DuelEvent._Buffer[2], i, 0);
					}
				}
	...
	case Final:
	{
		...
		sprintf_s(DuelEvent._Buffer[0], "%s é o campeão do Mata-Mata!", lpObj->Name);
		sprintf_s(DuelEvent._Buffer[1], this->_Notice);
...
bool cDuelEvent::Attack(LPOBJ lpObj, LPOBJ Target)
{
	...
	if (this->_State != Empty)
	{
		if (this->_Selected[0].lpObj == lpObj)
		{
			if (this->_Selected[0].Select == true)
			{
				...
				gObjTeleport(this->_Selected[0].lpObj->m_Index, 0, 125, 125);
		...
		else if (this->_Selected[1].lpObj == lpObj)
		{
			if (this->_Selected[1].Select == true)
			{
				...
				gObjTeleport(this->_Selected[1].lpObj->m_Index, 0, 125, 125);
