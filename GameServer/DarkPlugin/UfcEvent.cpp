...
cUfcEvent::cUfcEvent() : _State(Empty)
{
}
...
bool cUfcEvent::Load()
{
	...
	this->_Players[0] = std::vector<UfcPlayer>();
	this->_Players[1] = std::vector<UfcPlayer>();
	...
	Token.ParseFile(std::string("./Data/UfcEvent.txt"), Group);
...
bool cUfcEvent::Check(LPOBJ lpObj, const char* Text)
{
		...
		if (!_stricmp(Text, this->_Syntax[0]))
		{
			if (this->_State == Register && lpObj->Live == TRUE)
			{
				if (this->_Type != 4 && lpObj->Class != this->_Type)
				{
					...
					{
					case 0:
						Func.MsgOutput(lpObj->m_Index, 1, "Ufc exclusivo para Dark Wizards e Soul Masters");
						break;
					case 1:
						Func.MsgOutput(lpObj->m_Index, 1, "Ufc exclusivo para Dark Knights e Blade Knights");
						break;
					case 2:
						Func.MsgOutput(lpObj->m_Index, 1, "Ufc exclusivo para Elfs e Muse Elfs");
						break;
					case 3:
						Func.MsgOutput(lpObj->m_Index, 1, "Ufc exclusivo para Magic Gladiators");
						break;
					}
...
void cUfcEvent::Start(int Time, int Class)
{
	if (Time == 0)
	{
		Time++;
...
void cUfcEvent::Run()
{
	...
	case Register:
	{
		if (this->_Count == 0)
		{
			...
			else
			{
				...
				sprintf_s(UfcEvent._Buffer[0], "Inscrições para o evento encerradas.");
				sprintf_s(UfcEvent._Buffer[1], "PRIMEIRA RODADA!");

				for (int i = OBJECT_MIN; i < OBJECT_MAX; i++)
				{
					if (gObj[i].Connected == 3)
					{
						GCServerMsgStringSend(UfcEvent._Buffer[0], i, 0);
						GCServerMsgStringSend(UfcEvent._Buffer[1], i, 0);
					}
				}
		...
		else
		{
			if ((this->_Count % 60) == 0)
			{
				sprintf_s(UfcEvent._Buffer[0], "Ufc começará em %d Minuto(s)", (this->_Count / 60));
				sprintf_s(UfcEvent._Buffer[1], "Para participar, utilize o comando: %s", this->_Syntax[0]);
	...
	case Select:
	{
		if (this->_Count == 0)
		{
			...
			SelectWO:
				...
				sprintf_s(UfcEvent._Buffer[0], "[   UFC   ]");

				sprintf_s(UfcEvent._Buffer[1], "%s venceu de W.O.", this->_Selected[0].lpObj->Name);
			...
			default:
			{
				...
				else
				{
					...
					sprintf_s(UfcEvent._Buffer[1], "%s  vs  %s", this->_Selected[0].lpObj->Name, this->_Selected[1].lpObj->Name);
	...
	case Died:
	{
		if (this->_Count == 0)
		{
			...
			else
			{
				...
				sprintf_s(UfcEvent._Buffer[0], "[    PLACAR    ]");

				sprintf_s(UfcEvent._Buffer[1], "%s  vs  %s", this->_Selected[0].lpObj->Name, this->_Selected[1].lpObj->Name);

				sprintf_s(UfcEvent._Buffer[2], "%d   x   %d", this->_Selected[0].Score, this->_Selected[1].Score);
				...
				for (int i = OBJECT_MIN; i < OBJECT_MAX; i++)
				{
					if (gObj[i].Connected == 3)
					{
						GCServerMsgStringSend(UfcEvent._Buffer[0], i, 0);
						GCServerMsgStringSend(UfcEvent._Buffer[1], i, 0);
						GCServerMsgStringSend(UfcEvent._Buffer[2], i, 0);
					}
				}
	...
	case Final:
	{
		...
		Manager.ExecFormat(Query._Query[36], lpObj->Name);
		Manager.ExecFormat(Query._Query[37], lpObj->AccountID);

		sprintf_s(UfcEvent._Buffer[0], "%s é o campeão do Ufc!", lpObj->Name);
		sprintf_s(UfcEvent._Buffer[1], this->_Notice);
...
bool cUfcEvent::Attack(LPOBJ lpObj, LPOBJ Target)
{
...
void cUfcEvent::Die(LPOBJ lpObj)
{
...
void cUfcEvent::Quit(LPOBJ lpObj)
{
	if (this->_State != Empty)
	{
		if (this->_Selected[0].lpObj == lpObj)
